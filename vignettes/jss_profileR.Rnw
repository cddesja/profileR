%\VignetteIndexEntry{Profile Analysis of Multivariate Data in R: An Introduction to the profileR Package}
...
\begin{equation}
T^2 = n\mathbf{(\boldsymbol{\bar{x}}-\boldsymbol{\mu_0})'S^{-1}(\boldsymbol{\bar{x}}-\boldsymbol{\mu_0})}
\end{equation}
...
\begin{equation}
T^2 = n\mathbf{(\boldsymbol{\bar{x}}-\boldsymbol{\mu_0})'\Sigma^{-1}(\boldsymbol{\bar{x}}-\boldsymbol{\mu_0})}.
\end{equation}
...
\begin{equation}
T^2=N(\mathbf{\bar{X}_{tot}})'(\mathbf{S_{wg}})^{-1}(\mathbf{\bar{X}_{tot}})
\end{equation}
...
\begin{table}[h!]
\begin{center}
\caption[]{Inventory of Personality and Mood Manifestation.}
\begin{tabular}{lllll}
\hline
A & H & S & B & C \\
\hline
...
\begin{equation}
\rho_p=\frac{(\overrightarrow{T}_i-\overrightarrow{T}_{i.})^\text{T}\mathbf{A}(\overrightarrow{T}_i-\overrightarrow{T}_{i.})}{(\overrightarrow{X}_i-\overrightarrow{X}_{i.})^\text{T}\mathbf{A}(\overrightarrow{X}_i-\overrightarrow{X}_{i.})},
\end{equation}
...
\begin{equation}
\hat{\rho}_W=\frac{\sum_{p=1}^{P}\left(\sum_{v=1}^{V}(X_{pv}-\bar{X}_p)(X_{pv'}-\bar{X}_{p'})\right)}{\sum_{p=1}^{P}(\sum_{v=1}^{V}(X_{pv}-\bar{X}_p)^2}, \text{and}
\end{equation}
...
It should be noted that the within-person reliability coefficient is a weighted average of within-person reliability coefficients from all subtests. Without averaging over the subtests, within-person reliability coefficients can also be computed to evaluate reliability for each subtest (for more details, see \citet{bulut2013between}). 
...
The first four columns are the items and the last column in the data is the group variable. We will run \code{pbg} on this data set and store it in an object called \code{mod}. 
\begin{CodeChunk}
\begin{CodeOutput}
...
set.seed(2015)
male <- mvrnorm(n = 500, rep(0, 3), Sigma)
female <- mvrnorm(n = 500, c(0.1, 0.2, 0.3), Sigma)
data <- as.data.frame(rbind(male, female))
...
\begin{CodeChunk}
\begin{CodeOutput}
> mod2 <- pbg(data[,1:3],data[,4],labels=FALSE,profile.plot=TRUE)
> print(mod2)
...
\begin{tabular}{ll}
\hline
\code{R} output & Null hypothesis test  \\
\hline
...
\begin{CodeInput}
set.seed(13251)
dv <- rbinom(n = 100, size = 1, prob = mean(IPMMc$R))
A <- rnorm(n = 100, mean = mean(IPMMc$A))
H <- rnorm(n = 100, mean = mean(IPMMc$H))
S <- rnorm(n = 100, mean = mean(IPMMc$S))
B <- rnorm(n = 100, mean = mean(IPMMc$B))
dat0 <- data.frame(dv, A, H, S, B)
\end{CodeInput}
\end{CodeChunk}

To perform cross-validation, we call \code{pcv} and save the output in \code{cv.model} (which is also a \code{critpat} object):

\begin{CodeChunk}
\begin{CodeInput}
cv.model <- pcv(dv ~ A + H + S + B, data = dat0)
\end{CodeInput}
\end{CodeChunk}

As with \code{cpa}, \code{pcv} uses the same syntax as a call to \code{glm} does. Regression coefficients can be obtained through printing. 
...
\begin{CodeOutput}
Call:
pcv(formula = dv ~ A + H + S + B, data = dat0)

Coefficients
$`Random Sample 1`
            A             H             S             B 
 0.0514905676  0.0515755913 -0.0285983736  0.0005443284 

$`Random Sample 2`
           A            H            S            B 
-0.087656366 -0.013529506 -0.155730560 -0.007347574 
\end{CodeOutput}
\end{CodeChunk}

It may be useful to set the seed when doing cross-validation and this can be set through the \code{seed} argument.

\begin{CodeChunk}
\begin{CodeInput}
cv.model <- pcv(dv ~ A + H + S + B, data = dat0, seed = 4135)
\end{CodeInput}
\end{CodeChunk}

The proportion of variability associated with each effect can be obtained using \code{summary} and hypothesis tests (described in Table~\ref{tab:hyptest}) can be obtained using \code{anova} for each sample.
