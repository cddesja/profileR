...
pr <- function(form1,form2) {
  ...
  f1 <- as.matrix(rowMeans(form1, na.rm = TRUE), ncol = 1, 
                  nrow = k)
  f2 <- as.matrix(rowMeans(form2, na.rm = TRUE), ncol = 1, 
                  nrow = k)
  ...
  for (i in 1:n) {
    covar1[, i] = cov(form1[, i], form2[, i], use = "pairwise.complete.obs")
  }
  
  #Level reliability
  num1 = rowSums(covar1, na.rm = TRUE)
  variance.form1 <- sum(apply(form1, 2, function(x) var(x, na.rm = TRUE)), na.rm = TRUE)
  variance.form2 <- sum(apply(form2, 2, function(x) var(x, na.rm = TRUE)), na.rm = TRUE)
  denum1 <- sqrt(variance.form1 * variance.form2)
  overall <- num1/denum1
  num2 <- n * (cov(f1, f2, use = "pairwise.complete.obs"))
  ...
  for (i in 1:n) {
    covar2[, i] = cov(pattern1[, i], pattern2[, i], use = "pairwise.complete.obs")
  }
  
  num3 = rowSums(covar2, na.rm = TRUE)
  var.form1 <- sum(apply(pattern1, 2, function(x) var(x, na.rm = TRUE)), na.rm = TRUE)
  var.form2 <- sum(apply(pattern2, 2, function(x) var(x, na.rm = TRUE)), na.rm = TRUE)
